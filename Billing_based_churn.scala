...
import Logger.Log.Logging._

object Billing_based_churn {
  def main(args: Array[String]): Unit = {
    ...
    val Query1 = sparkSession.sql("select distinct(MonthlyCharges),customerID from telecom where Churn='Yes'").groupBy("MonthlyCharges").count().coalesce(1).write.option("header", "true").csv(config.getString("outputpath")+"_MonthlyCharges_based")
    val query2= sparkSession.sql("select distinct(TotalCharges),customerID from telecom where Churn='Yes'").groupBy("TotalCharges").count().coalesce(1).write.option("header", "true").csv(config.getString("outputpath")+"_TotalCharges_based")
