...
object Tenure_and_seniorcitizen_based_churn {
  def main(args: Array[String]): Unit = {
    ...
    val Query = sparkSession.sql("SELECT tenure,count(CASE WHEN Churn='yes' then tenure ELSE 0 END) ans FROM telecom GROUP BY tenure")
      .coalesce(1).write.option("header", "true")
      .csv(config.getString("outputpath")+"_Tenure_based")
  }
}
