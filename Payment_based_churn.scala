...
object Payment_based_churn {
  def main(args: Array[String]): Unit = {
    ...
    val Query = sparkSession.sql("Select * from telecom where Churn='Yes'").groupBy("PaymentMethod").count()
      .coalesce(1).write.option("header", "true").csv(config.getString("outputpath")+"_Payment_based")
